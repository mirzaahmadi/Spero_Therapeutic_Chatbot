...
gpt4o_model = ChatOpenAI(model_name="gpt-4o", temperature=0.2)
...
def deepseek_api_call(prompt):
    """
    Sends a user prompt to the DeepSeek API and returns the model's response, handling errors if they occur.
    Used for evaluatemodels.py for the case that models == gpt4o'
    """
    # Set up the headers for the HTTP request, including the API key and title
    headers = {
        "Authorization": f"Bearer {DEEPSEEK_API_KEY}", 
        "X-Title": "spero-eval"  
    }

    # Set up the data to be sent in the request. This includes the model choice and the user prompt.
    data = {
        "model": "deepseek-chat",  
        "messages": [
    ...
    # Send a POST request to the DeepSeek API, passing the headers and data
    response = requests.post("https://openrouter.ai/api/v1/chat/completions", headers=headers, json=data)

    # Check if the response was successful (HTTP status code 200)
    if response.status_code == 200:
        # If successful, return the model's response content from the API response
        return response.json()["choices"][0]["message"]["content"]
    else:
        # If there's an error, print the error status and response text
        print(f"❌ DeepSeek error: {response.status_code}, {response.text}")
        return "Error: DeepSeek failed."

# === GET MODEL RESPONSE FUNCTION ===
def get_model_response(prompt, model="gpt4o"):
    """
    Calls the selected language model (GPT-4o or DeepSeek) with the given prompt.
    """
...
chat_interface = gr.ChatInterface(
    fn=stream_response, 
    title="Hi, I'm Spero 🕊️. What's on your mind?",
